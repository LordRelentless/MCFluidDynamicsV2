...
function setBlock(map: Map<string, VoxelData>, x: number, y: number, z: number, color: number, type: 'solid' | 'water' = 'solid') {
...
// Helper to fill a 10x10x10 volume (Macro Block)
function fillMacroBlock(map: Map<string, VoxelData>, bx: number, by: number, bz: number, color: number, type: 'solid' | 'water' = 'solid') {
    const startX = bx * 10;
    const startY = CONFIG.FLOOR_Y + 1 + (by * 10);
    const startZ = bz * 10;

    for(let x = 0; x < 10; x++) {
        for(let y = 0; y < 10; y++) {
            for(let z = 0; z < 10; z++) {
                // If water, only fill center to allow expansion, or fill all?
                // Filling all 1000 makes it very dense.
                setBlock(map, startX + x, startY + y, startZ + z, color, type);
            }
        }
    }
}
...
export const Generators = {
    ...
    WaterTank: (): VoxelData[] => {
        ...
        // --- Minecraft Source Block Simulation ---
        const floorY = CONFIG.FLOOR_Y + 1;
        const width = 12;
        const depth = 8;
        const containerHeight = 15;
        
        // 1. The Container (Aquarium)
        for(let x = -width; x <= width; x++) {
            for(let z = -depth; z <= depth; z++) {
                setBlock(map, x, floorY, z, COLORS.GLASS);
            }
        }
        // Walls
        for(let y = floorY; y <= floorY + containerHeight; y++) {
             for(let x = -width; x <= width; x++) {
                 setBlock(map, x, y, -depth, COLORS.GLASS); // Back
             }
             for(let z = -depth; z <= depth; z++) {
                 setBlock(map, -width, y, z, COLORS.GLASS); // Left
                 setBlock(map, width, y, z, COLORS.GLASS);  // Right
             }
             // Front glass (partial)
             for(let x = -width; x <= width; x+=3) {
                 setBlock(map, x, y, depth, COLORS.GLASS);
             }
        }

        // 2. The Source Block (10x10x10 = 1000 Voxels)
        const sourceY = floorY + 12;
        const sourceX = 0;
        const sourceZ = -4;

        for(let y = sourceY; y < sourceY + 10; y++) {
            for(let x = sourceX - 5; x < sourceX + 5; x++) {
                for(let z = sourceZ - 5; z < sourceZ + 5; z++) {
                     setBlock(map, x, y, z, COLORS.WATER, 'water');
                }
            }
        }

        // 3. Obstacles
        generateSphere(map, 0, floorY+1, 0, 3, COLORS.SAND);
        generateSphere(map, -6, floorY+4, -2, 2, COLORS.GLASS);
        generateSphere(map, 6, floorY+2, -2, 2, COLORS.GLASS);
    ...
    Terrain: (): VoxelData[] => {
        ...
        // Terrain Size in Macro Blocks
        const sizeX = 8; // Total 16 blocks wide
        const sizeZ = 8;
        
        // Generate Terrain
        for(let bx = -sizeX; bx <= sizeX; bx++) {
            for(let bz = -sizeZ; bz <= sizeZ; bz++) {
                
                // Heightmap function (Hills and Valleys)
                // Diagonal Valley
                const valleyFactor = Math.abs(bx + bz) * 0.8;
                // General Noise
                const noise = Math.sin(bx * 0.5) + Math.cos(bz * 0.5);
                
                let height = Math.floor(4 + noise * 2 + valleyFactor * 0.5);
                
                // Cap height
                if (height < 1) height = 1;
                if (height > 8) height = 8;

                // Create Basin (Dip) at center
                ...
                for(let h = 0; h < height; h++) {
                    ...
                    if (h === height - 1) color = COLORS.GRASS;
                    else if (h >= height - 3) color = COLORS.DIRT;
                    
                    fillMacroBlock(map, bx, h, bz, color, 'solid');
                }
            }
        }

        // Place Water Source on a high point
        fillMacroBlock(map, -sizeX + 1, 8, -sizeZ + 1, COLORS.WATER, 'water');
