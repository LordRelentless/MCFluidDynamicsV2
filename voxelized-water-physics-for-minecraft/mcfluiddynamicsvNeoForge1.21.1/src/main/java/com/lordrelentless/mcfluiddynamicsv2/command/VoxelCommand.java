...
public final class VoxelCommand {
    ...
    // This overload is used when a position is provided
    private static int generate(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
        String name = StringArgumentType.getString(context, "name");
        BlockPos pos = BlockPosArgument.getLoadedBlockPos(context, "pos"); // Can throw
        return generate(context, pos, name);
    }

    // Main logic - now accepts name as parameter to avoid looking it up twice
    private static int generate(CommandContext<CommandSourceStack> context, BlockPos pos, String name) throws CommandSyntaxException {
    ...
    // Overload used when no position is given (uses player position)
    private static int generate(CommandContext<CommandSourceStack> context, BlockPos pos) throws CommandSyntaxException {
        String name = StringArgumentType.getString(context, "name");
        return generate(context, pos, name);
    }
