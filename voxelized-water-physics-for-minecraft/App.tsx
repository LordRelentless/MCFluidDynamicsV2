...
const App: React.FC = () => {
  ...
  const handleNewScene = (type: 'Eagle' | 'WaterTank' | 'Terrain') => {
    // @ts-ignore - type checking dynamic access
    const generator = Generators[type];
    if (generator && engineRef.current) {
      engineRef.current.loadInitialModel(generator());
      
      let name: string = type;
      if (type === 'WaterTank') name = 'Water Experiment';
      if (type === 'Terrain') name = 'Terrain Demo';
  ...
  const handleToggleFluid = () => {
      engineRef.current?.startFluidSim();
  }

  const handleTemperatureChange = (val: number) => {
      setTemperature(val);
      engineRef.current?.setTemperature(val);
  }

  const handlePrecipitationChange = (val: number) => {
      engineRef.current?.setPrecipitation(val);
  }
  ...
  return (
    <div className="relative w-full h-screen bg-[#f0f2f5] overflow-hidden">
      ...
      <UIOverlay 
        ...
        onToggleFluid={handleToggleFluid}
