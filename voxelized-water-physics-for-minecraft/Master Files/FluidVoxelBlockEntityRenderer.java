...
public class FluidVoxelBlockEntityRenderer implements BlockEntityRenderer<FluidVoxelBlockEntity> {
    ...
    private static void quad(VertexConsumer vc, Matrix4f mat,
        ...
        vc.vertex(mat, x0, y0, z0).color(r, g, b, a).uv(0, 0).uv2(light).normal(nx, ny, nz).endVertex();
        vc.vertex(mat, x1, y0, z0).color(r, g, b, a).uv(1, 0).uv2(light).normal(nx, ny, nz).endVertex();
        vc.vertex(mat, x1, y1, z1).color(r, g, b, a).uv(1, 1).uv2(light).normal(nx, ny, nz).endVertex();
        vc.vertex(mat, x0, y1, z1).color(r, g, b, a).uv(0, 1).uv2(light).normal(nx, ny, nz).endVertex();
    }
